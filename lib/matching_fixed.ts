...
function calculateMatchScore(
  ...
  // Course/Branch matching (25% weight)
  let courseMatched = false;
  for (const userCourse of userProfile.courses) {
    if (isCourseMatch(userCourse, alumni.branch, alumni.course)) {
      courseMatched = true;
      break;
    }
  }
  ...
  if (courseMatched) {
    ...
    // Partial match if branch is similar
    const normalizedUserCourses = userProfile.courses.map(c => normalizeBranch(c));
    const normalizedAlumniBranch = normalizeBranch(alumni.branch);
    for (const userCourse of normalizedUserCourses) {
      if (normalizedAlumniBranch.includes(userCourse) || userCourse.includes(normalizedAlumniBranch)) {
        score += 12.5;
        break;
      }
    }
